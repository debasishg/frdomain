...
object Account {
  ...
  case class InvalidInterestRate(message: String) extends AccountException(message)
  ...
  private def validateRate(rate: BigDecimal) =
    if (rate <= BigDecimal(0)) InvalidInterestRate(s"Interest rate $rate must be > 0").failureNel[BigDecimal]
  ...
  private def validateAccountAlreadyClosed(a: Account) = {
    if (a.dateOfClose isDefined) AlreadyClosed(s"Account ${a.no} is already closed").failureNel[Account]
    else a.successNel[AccountException]
  }

  private def validateCloseDate(a: Account, cd: Date) = {
    if (cd before a.dateOfOpen.get) InvalidOpenCloseDate(s"Close date [$cd] cannot be earlier than open date [${a.dateOfOpen.get}]").failureNel[Date]
