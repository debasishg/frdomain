...
trait TradeModel {this: RefModel with ExecutionModel =>
  ...
  def allocateCustomerTrades(accounts: List[Account]): Execution => Valid[Trade] = { execution =>
    val q = execution.quantity / accounts.size
    ListT[StringOr, Trade](
      accounts.map { account =>
        makeTrade(account, execution.instrument, "t-123", execution.market, execution.unitPrice, q).right[String]
      }.sequenceU
    )
  }
}
